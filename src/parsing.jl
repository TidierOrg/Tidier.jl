# Not exported
function parse_tidy(
  tidy_expr::Union{Expr,Symbol,Number};
  autovec::Bool=true,
  subset::Bool=false,
  from_across::Bool=false,
) # Can be symbol or expression
...
function parse_autovec(tidy_expr::Union{Expr,Symbol})
  ...
  autovec_expr = MacroTools.postwalk(tidy_expr) do x
    ...
    elseif @capture(x, fn_(args__))
      ...
      if fn in
        [:Ref :Set :Cols :(:) :∘ :repeat :across :desc :mean :std :var :median :first :last :minimum :maximum :sum :length :skipmissing :quantile :passmissing]
        return x
...
function parse_escape_function(rhs_expr::Union{Expr,Symbol})
  rhs_expr = MacroTools.postwalk(rhs_expr) do x
    if @capture(x, fn_(args__))
      ...
      if fn in
        [:in :∈ :∉ :Ref :Set :Cols :(:) :∘ :across :desc :mean :std :var :median :first :last :minimum :maximum :sum :length :skipmissing :quantile :passmissing :startswith :contains :endswith]
        return x
      elseif contains(string(fn), r"[^\W0-9]\w*$") # valid variable name
        return :($(esc(fn))($(args...)))
      else
...
function parse_interpolation(
  var_expr::Union{Expr,Symbol,Number,String}; summarize::Bool=false
)
  found_n = false
  ...
  var_expr = MacroTools.postwalk(var_expr) do x
    ...
    elseif hasproperty(x, :head) &&
      x.head == :macrocall &&
      hasproperty(x.args[1], :mod) &&
      hasproperty(x.args[1], :name) &&
      x.args[1].mod == Core &&
      x.args[1].name == Symbol("@cmd")
